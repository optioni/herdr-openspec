...
lines=$(printf '%s\n' "$body" | wc -l | tr -d ' ')
echo "WIRED OK: nine names present in $MOD; run resolves state::state_dir; $MOD names config.agent_kind; 'pub fn run()' is $lines lines with no branch and no loop; no \"herdr\" and no \"claude\" literal under $UIDIR"
