...
| SUGGESTION | specs/tasks-checklist/spec.md | "An item's body is drawn under it at its hanging indent" ends on "the 58-column call produces strictly more body rows than the 78-column call", which its **own** WHEN fixture cannot satisfy: the body reflows to forty-one characters, which fit one row at both a 50-column body (`58`) and a 70-column one (`78`). Measured at implementation. The clause is the only one in the scenario asserting the body reflows at all, so leaving it would have forced either a failing test or a fixture silently swapped for one the spec does not name. | Clause split in two: the measured one-row-at-both-widths fact is stated for this fixture, and the reflow property moved to a second clause over the same item carrying a body long enough to wrap at both widths — which is what `an_items_body_is_drawn_under_it_at_its_hanging_indent` now asserts in its second half. Neither the hang rule nor any other scenario moves. | `specs/tasks-checklist/spec.md` → "An item's body is drawn under it at its hanging indent" |

| CRITICAL | specs/tasks-checklist/spec.md | The live requirement "A source holding no task lines renders `No tasks yet`" (`openspec/specs/tasks-checklist/spec.md:405-410`) states the checklist renders the bar, its blank line, and **exactly one further line** reading `No tasks yet`. Task 5.3a's chosen branch draws every group's retained blocks beneath that row (`src/ui/tasks.rs:611-625`, `db57226`), and no delta in this change reached that requirement — so at archive a live requirement would state something the code contradicts. Found by the Change Review; same class as gaps C4 and C6, which planning review filed against `artifact-content` and `artifact-folds` for the same reason. | Requirement added to the MODIFIED set, re-derived from the live text by extraction plus `diff -u`: two hunks, the amended sentence with the reasoning for the change, and one new scenario "A prose-only document draws its blocks beneath `No tasks yet`". The only removed line is the "exactly one further line" sentence itself; all five carried scenarios are byte-identical. The companion clause "SHALL render **no** heading line" is **unchanged and still true** — `group_body` draws no heading and the heading loop sits after the early return — and the delta says so rather than leaving a reader to check. | `specs/tasks-checklist/spec.md` → "A source holding no task lines renders `No tasks yet`" |

| WARNING | tasks.md, design.md | Group 5's two scenarios are filed in `design.md` -> Verification Matrix as tier **V** (`TestBackend`, `cargo test --lib ui::view`) and `tasks.md` -> Ordering calls them "the end-to-end tier", but both were implemented as unit tests over `content_lines` with hand-built `Detail` values: nothing synced, the injected reader never called, and no body or block row ever painted into a buffer. The likely driver is gate placement (`DETAILWIDTHS` wants 58/78, `WIDTHS` wants 60/120), which is a legitimate reason to move a test but went unrecorded — leaving the sync -> sections -> walk -> frame path, which is group 5's *only* risk, untested end to end. | Two frame-level tests added in `src/ui/view.rs` through a **synced** dashboard at 120x20 and 60x20, the `ui::detail` pair kept for the `ContentKind` and `section_at` assertions a buffer cannot show. Confirmed falsifiable by planting group 4's items-only flatten: the open-section test fails. | `src/ui/view.rs`; sibling tests in `src/ui/detail.rs` unchanged |
| WARNING | specs/artifact-folds/spec.md | Both new scenarios describe a fixture of **one** `## 1. Setup` heading. Measured at implementation: one heading yields one section, which has nothing to fold against, so `content_lines` takes the non-foldable `ui::tasks::lines` grammar — the tab renders `## 1. Setup` as a literal heading line, `detail.expanded` changes nothing, and "Collapsing a tracked-tasks section hides its bodies and blocks with its items" cannot be exercised at all. Same class as the body-reflow clause repaired earlier in this change: a scenario whose own fixture cannot produce the state it asserts over. | Both WHENs gain a second `## 2. Build` section, with a clause recording **why** it is part of the fixture rather than incidental, so a later reader does not trim it back to the minimal-looking one-heading form. | `specs/artifact-folds/spec.md` -> the two new scenarios |
