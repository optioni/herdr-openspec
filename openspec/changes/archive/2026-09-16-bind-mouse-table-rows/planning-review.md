...
Reviewer B's full findings arrived after the first commit of this package; its six additional
findings are repaired in the rows below and were committed as a follow-up.
...
| WARNING | tasks.md | The planting script's row carries no zone, so after the change it would exit 101 via the *missing-zone* rule, never reaching the vacuity leg — the headline control would fire for the wrong reason. | The plant now carries `Zone::DetailRow` and `Target::DetailLine`, **replaces** the real row, and the task asserts on the failure message, not only the exit status. | tasks.md 5.8 |
| WARNING | tasks.md | Task 4.4 named `the_claim_count_matches_every_document`, which does not exist. | Renamed to `documented_claim_count_matches_the_file` (`tests/doc_contract.rs:3288`). | tasks.md 5.6 |
...
| WARNING | tasks.md | Group 4 (now 5) had neither a REFACTOR task nor the statement that none was needed. | The deferred fold landed there as 5.7. | tasks.md 5.7 |
...
| WARNING | design.md | `ui::layout::zone` — the change's newest real collaborator — was absent from Test Boundaries, and no artifact said where a claim's zone comes from. `mouse_action` never returns it, so the sweep must recompute it and mirror the help-open and out-of-frame precedence. Reviewer B had to write that code to produce its dumps. | Decision 11 added, a boundary row added, and the two precedence rules stated normatively in the spec. | design.md → Decision 11 and Test Boundaries; specs → "Where the zone comes from"; tasks.md 1.2 |
| WARNING | tasks.md, specs/doc-conformance | "The catch-all claims at least one" had no negative control and could never fire against the real tree, where ~86 of 104 claims are `Ignore`. An assertion that cannot go red is not a guard. | A synthetic-claim-set control added as its own RED task, scenario and matrix row. | tasks.md 5.4; specs → empty-catch-all scenario; design.md → matrix |
| WARNING | tasks.md | Task 1.1 asserted a **type-level** property the compiler already enforces — once `sweep_mouse_claims` returns claims, no planted defect short of a compile error makes it red, and a zone/action mix-up passes it. | Replaced with three concrete adversarial assertions, one of them an **absence**. | tasks.md 1.1 |
| NOTE | design.md | Decision 2 enumerated two payload-carrying `Zone` variants; there are three — `DetailTab` was omitted. The conclusion was unaffected but the list read as exhaustive. | All three named, with the consequence for the tab-cell row stated. | design.md → Decision 2 |
| NOTE | design.md | Test Boundaries said "three frames"; the sweep is three frames × two overlay states, and after the fixture axis × N fixtures. | Corrected. | design.md → Test Boundaries |
| NOTE | tasks.md | The fixture axis **adds** claims the table must then cover, so it is a table edit too — not enumerated anywhere before the table-writing group. | An enumeration task added, feeding group 4. | tasks.md 2.3 |

Reviewer B also supplied the measured zone set per active outcome under the repaired key
(dump 2: 105 claims, 19 active, thirteen distinct gesture/outcome pairs). Group 4 now writes
the table's zone sets from that measurement rather than by inference, and the dump confirms the
repair works: `Click(DetailHeader)` and `Select(Begin)` are distinct claims, so the stale row is
genuinely vacuous under axis 4 where it was not under the bare name.
