...
**Reason**: This is the change that adds them. `repo-foundation` withheld the `[[actions]]`
table and the `dashboard-tab` pane because the `open` subcommand they invoke did not exist
and Herdr's menu would have offered entries that exit 2; `plugin-actions` ships that
subcommand, so the reason for the absence is gone. The requirement is removed rather than
modified because its whole content is the negative claim.

**Migration**: None for any consumer. Anyone reading the manifest sees two `[[actions]]`
entries and two `[[panes]]` entries where there were none and one; the `dashboard` pane's
id, title, placement, and command are unchanged, so an existing
`herdr plugin pane open --plugin herdr-openspec --entrypoint dashboard` keeps working.
