...
- This repository HEAD at review time: `857163a13178f03b61fd6754b9df0106a58d7128`
- **Re-checked during implementation** at `d2f7e31` (end of task group 3) `389317a` (end of task group 4) `22839cd` (end of task group 6) and `5447bf7` (end of task group 7). The intervening
  commits are this change's own planning artifacts and its groups 0-3; no external commit
  touched a planned contract or implementation surface. Two repairs were made at that point
...
Found by implementation rather than by review, and repaired in the artifact that owns them
before the next group started, per the apply workflow's Drift rule.

| Found at | Artifact | Problem | Repair |
|---|---|---|---|
| Group 3 GREEN | `specs/artifact-folds/spec.md`, `specs/artifact-content/spec.md`, `tasks.md` | GAP-06. The spec-glob fixture was called a **six**-section one in six places, and its depth list written `0, 1, 2, 3, 2, 0`, while the scenario at `artifact-folds` → "A spec glob nests requirements under their capability" **enumerates seven**: a file section, four heading sections, and two unsplit siblings. Three paths resolve and `## MODIFIED Requirements` does not split, so seven is what the derivation rule produces; the enumeration was right and every count of it was one short. Separately, `artifact-content` → "No `content_lines` line exceeds its width at any width" still said "the same **nine** `Detail` values" after this change's delta added two to that list. | Count corrected to **seven** at all six sites and the depth list to `0, 1, 2, 3, 2, 0, 0`; the nine corrected to **eleven**. No behaviour changed — group 3 had already implemented and asserted the enumerated list. |
| Group 4 RED | `specs/artifact-folds/spec.md` | GAP-08. "A narrow pane truncates the label and keeps the glyph" gave the depth-2 header's narrow literal as `    > Requi…`, which measures **twelve** display columns at a thirteen-column region. Every sibling literal in the same scenario is an exact fit, and `ui::list::pad_or_truncate_right` — the crate's one truncation rule — pads or truncates every header row to exactly the region width, so a twelve-column row is not a value the code can produce. | Corrected to `    > Requir…`, thirteen columns. A one-character repair; group 4 had already implemented and asserted the thirteen-column form. |
| Group 3 VERIFY | `tasks.md` groups 6.1 and 6.6 | GAP-07. Group 3's split gate turned `src/ui/driver.rs`'s shared `twenty_task_source` fixture (`## Tasks` plus twenty items) into a one-section artifact: the heading became the section's `label`, the tab stayed non-foldable, and no header row was drawn, so `checklist_scroll_is_clamped`'s clamp moved 9 → 8 **and coincided with the markdown body's own**, collapsing the `assert_ne!` that is the test's entire point. `tab_move_resets_and_reclamps` failed on the same fixture's precondition. The plan scheduled both repairs in group 6, which would have left `make check` red across groups 4 and 5 — the exact window `tasks.md`'s own commit-discipline note was written to avoid — and 6.6 additionally listed `tab_move_resets_and_reclamps` as passing **unedited**, which it does not. | The fixture is now **headingless**, which is task 6.1's stated offset-rule half landed early and verbatim; both tests are green at group 3. Task 6.1 narrowed to the cursor-rule half (twenty items under two headings) it still owns; task 6.6 no longer claims `tab_move_resets_and_reclamps` is unedited. No design decision moved: this is Decision 10's stated one-section edge, whose fallout the task sequence had not scheduled. |

| Group 6 GREEN | `specs/tasks-checklist/spec.md` | GAP-09. "The tasks tab shows checkboxes and its siblings show markdown" gave its source as a **one-group** task file and then required the render to show `v 1. Setup` as a fold header. One group splits into exactly one section, which `Detail::foldable` reports not foldable (design.md -> Decision 10), so no header row is drawn and the THEN is unsatisfiable by the WHEN. | Source changed to two groups, and the scenario now says in as many words why one will not do. Group 6 had already implemented against a two-group fixture. |
| Group 6 RED | `specs/detail-scroll/spec.md` | GAP-10. "Scrolling the checklist is clamped against the checklist's own length" scripted **twenty** `j` presses over a body that draws twenty-five rows, so the cursor clamp at `24` never binds: twenty presses land at exactly `20`, satisfying neither the "row count less one" clause nor the "not to `20`" one. Its THEN also said "under one heading" where its own WHEN says two. | Press count corrected to **thirty**, the row arithmetic written into the scenario, and the "one heading" corrected to "two". |
| Group 6 RED | `specs/detail-scroll/spec.md` | GAP-11. "`j` walks the groups rather than scrolling the lines" called `detail.scroll` of `4` "the second group's header row". It is the first group's **second item row**; the second group's header sits at row `6`, behind the two item rows and the blank separator. The literal `4` is right and the description of it was wrong. | Description corrected, and row `6` added as its own clause — the same fact seen from the row list rather than the cursor. The landed test asserts both. |

| Group 7 RED | `specs/artifact-folds/spec.md` | GAP-12. "`Space` on a preamble row is inert" called the preamble rows "any row from the progress bar through the last line of `Intro prose.`". On a tracked-tasks tab `Intro prose.` draws **no** row: a `None`-labelled section's body there goes through `ui::tasks::items`, which renders task items and nothing else. The preamble rows are the bar row and its blank line, and those two only. | Scenario corrected to name the real two rows and to say why the prose line is not among them. |
| Group 7 RED | `specs/mouse-input/spec.md` | GAP-13. "A click on a task group's header folds that group" said a press on the progress-bar row or its blank line returns `Action::Ignore`. Its own requirement **table**, directly above, sends every drawn row of a foldable content area that is not a header to `Action::Click(Target::DetailLine(line))` — and the sibling scenario two entries down relies on exactly that. The scenario contradicted the normative half of its own requirement. | Scenario corrected to the table's answer, with the consequence spelled out: the press moves `detail.scroll` and folds nothing, and `Space` from where it lands is inert. Production was not bent to the prose. |

**The pattern behind GAP-07 and GAP-09**, recorded because it cost three repairs across three groups: a fixture written as the *smallest* thing that exercises the feature — one heading, one group — is **not foldable** under Decision 10, which needs more than one section. Every scenario in this capability that means to exercise folding needs at least two. GAP-06, GAP-08, GAP-10, GAP-11 and GAP-12 are a second, unrelated pattern: hand-computed counts, literals, row sets and press counts that no test had been run against until implementation ran one. GAP-13 is a third and rarer kind — a scenario contradicting the normative table of its own requirement, which is the one species of defect a reader checking scenarios against each other would never catch, because both halves were internally consistent with everything except each other.
