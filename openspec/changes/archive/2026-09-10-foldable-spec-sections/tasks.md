...
- [x] 11.5 **Done, in the same session as the archive as required.** `artifact-folds`' `## Purpose` is written: the archiver's `TBD - created by archiving change` placeholder was in place and `tests/spec_purposes.rs` was red on it, exactly as this task predicted; the Purpose replaced it and the test is green (3 passed), with `openspec validate --specs --strict` at 45 items passed. The three now-false Purposes were rewritten before the archive.
