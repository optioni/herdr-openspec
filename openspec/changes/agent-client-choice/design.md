...
|---|---|---|
| `herdr` binary | real, as a scratch shell program that logs every argument vector and scripts its answers | replaced — `integration::parse`/`resolve` take `&str` and slices; `decide` takes plain values |
...
|---|---|---|---|---|
| `agent-launch` :: Two installed integrations stop the launch with one problem and no pane | invocation log of exactly one entry; state dir byte-identical | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
...
| `agent-launch` :: A configured kind suppresses the stop entirely | same fixture with `agent_kind` set; four log entries | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
...
| `agent-launch` :: The three calls appear in order with the split's own pane id | invocation log of four entries, incl. a non-UTF-8 root run | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `agent-launch` :: Each intent sends its own `/opsx:*` command and nothing else | three launches in one worker; prompt element per intent | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `agent-launch` :: An archived change launches on the same terms as an active one | `Dashboard::apply` + the worker, against the active case | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `agent-launch` :: A failed split leaves nothing behind | scratch `herdr` exits 1 on `pane split` | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `agent-launch` :: A failed start leaves the pane and names it | scratch `herdr` exits 1 on `agent start` | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `agent-launch` :: A failed prompt leaves a running, un-prompted agent that is still attributable | scratch `herdr` exits 1 on `agent prompt` | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `agent-launch` :: A failed recording does not undo a successful start | state directory planted as a regular file | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `agent-launch` :: A failed recording and a failed prompt are both reported | both plants at once; `problems` order asserted | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `agent-launch` :: Two resolution problems, a failed recording, and a failed prompt are all four reported | all plants at once; four-entry `problems`; 17-line control | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `agent-launch` :: A success clears both entries | a clean launch following a four-problem one | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
...
| `agent-prompts` :: The prompt carries the probe's own path, not a bare command | invocation log's `agent prompt` element; fixture change name pinned | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
...
| `agent-prompts` :: `g` still works with no binary | worker driven with `Request::Focus` and `openspec_bin` `None` | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
...
| `integration-status` :: The first launch reads the status and the second does not | worker driven with two `Request::Launch`; invocation log | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `integration-status` :: Focus never reads the status | worker driven with one `Request::Focus`; invocation log | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
...
| `integration-status` :: A failed status call still launches the configured kind | scratch `herdr` exits 1 on `integration status` | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `integration-status` :: A failed status call with nothing configured reaches `claude` with two problems | same, with no configured and no recorded kind | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
| `integration-status` :: Unparseable output is not a failed call | three- and seventeen-line unparseable inputs; summary vs per-line counts | scratch-tree | scratch `herdr` program real at the seam; filesystem real | `cargo test --lib launch::` |
...
- **`Outcome::problems` growing to three pushes more rows above the change list.** → Bounded
  and stated, with a scenario that renders exactly three and asserts no fourth; three
  `! `-rows still leave a usable list at 20 rows.
