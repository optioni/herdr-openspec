...
- [ ] 7.5 CHANGE: **DO THIS AS PART OF `openspec archive`, NOT BEFORE — and nothing will catch it if the archive step forgets** (Change Review WARNING). `tests/spec_purposes.rs` rejects only an empty or placeholder Purpose, so all three phrases below pass every gate while being false. Grep for them verbatim rather than re-reading the sections:
