...
**Reason**: `help-overlay` adds a fifteenth field, `help: Help`, and this requirement's name
carries the count. The requirement is not withdrawn — it is re-stated under its new name in the
ADDED block below, with the field, the type, the `NODEFAULT-UI` set, and the pure-view file
list all corrected. Removing and re-adding is how `list-sections` made the same rename when the
count went from thirteen to fourteen, and it is done that way here for the same reason: an
OpenSpec requirement header is a merge key, so a renamed header must be withdrawn under the old
name and introduced under the new one rather than edited in place.
