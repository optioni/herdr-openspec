...
| WARNING | `specs/dashboard-loop` | Reported by the Change Review at its second pass, after the first round of repairs had landed. The pure-view scenario's closing bullet read "the check fails when any of the **eight** files is absent" while its own `WHEN` enumerates ten files at `:909-912`, its `THEN` says "no match in any of the ten", and a bullet two above says "it is ten here … rather than sweeping nine files". It survived five separate sweeps for pre-repair figures because it sits three bullets below a **historically correct** one — "the searched set was exactly eight after `agent-polling`, `agent-attribution`, and `agent-launch`" — so a grep for `eight` in that file looks like it has found the history and stopped. | Corrected to **ten**, with the two bullets' different subjects named in the text so the adjacency stops being a trap: the historical bullet names the set `agent-launch` left behind, this one names the set the check sweeps here. | `specs/dashboard-loop` (pure-view scenario) |

**A citation convention this change nearly broke twice.** Every figure in a delta written as
"N today", or otherwise describing the tree the change starts from, is verified against the
change's **base commit** and not against HEAD. `specs/dashboard-loop:774` says `make gates` runs
`nodefault-ui.sh` "**six** times today (`grep -c nodefault-ui.sh Makefile` = 6, lines 45-50)".
That is correct: `git show 0745e70^:Makefile` gives exactly six, first at line 45. At HEAD it
reads seven at lines 46-52, because this change's own `helpwidths.sh` line shifted the recipe.
A reader checking it against HEAD gets a false positive and "repairs" a true sentence into a
false one — which is strictly worse than the drift the check was looking for.
`specs/view-palette:110-111`'s nine/eight is the same shape, and defends itself by naming the
commit it was measured at (`08025d3`). That is the pattern worth copying: **a historical figure
should carry the commit or the change that makes it true**, so the next sweep can tell a
superseded number from a deliberate one without re-deriving the history.
