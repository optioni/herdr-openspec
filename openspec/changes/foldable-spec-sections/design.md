...
| detail-scroll :: A foldable tab is clamped to its last line, not its last screenful | `run_loop` at 120x40 and 60x40 with the same script; assert `scroll == 2` and the emphasis | loop | every collaborator replaced | `cargo test --lib ui::driver::tests::a_foldable_tab_is_clamped_to_its_last_line_not_its_last_screenful` |
