...
| artifact-folds :: A tab move forgets the fold, a forced reload does not | three dashboards, assert `expanded` after each path | unit (pure) | reader replaced, refresher replaced | `cargo test --lib ui::app::tests::keys::a_tab_move_forgets_the_fold_a_forced_reload_does_not` |
...
| detail-scroll :: At a collapsed foldable tab the same keys walk the section list | two `Next`s, assert `scroll` and the emphasised header at 120x40 and 60x40 | view | terminal replaced | `cargo test --lib ui::app::tests::at_a_collapsed_foldable_tab_the_same_keys_walk_the_section_list` |
...
| mouse-input :: A click on an artifact-section header folds it exactly as `Space` does | draw at 120x40, press row 1, assert the action and the field-for-field equality; repeat at `Route::List` | view | terminal replaced | `cargo test --lib ui::driver::tests::a_detail_header_click_equals_space` |
| mouse-input :: A click in an open section's body moves the detail cursor and folds nothing | press a body row, assert `DetailLine`, then a following `ToggleSection` | view | terminal replaced | `cargo test --lib ui::driver::tests::a_body_click_moves_the_cursor_and_folds_nothing` |
| mouse-input :: A click on a non-foldable tab's content is inert | three presses, assert `Ignore` and an unchanged dashboard | view | terminal replaced | `cargo test --lib ui::driver::tests::a_click_on_a_non_foldable_tab_is_inert` |
