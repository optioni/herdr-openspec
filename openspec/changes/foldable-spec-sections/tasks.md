...
| R5 — `Space` is route-blind today | `grep -n 'Action::ToggleSection => self.apply_toggle_section(),' src/ui/app.rs` | **exit 0** — the unconditional arm group **7** replaces. Line **433** at `08025d3`; **488** after groups 2, 3 and 5 added code above it, so group 7 must grep for the arm rather than trust the number |
...
- [x] 5.1 RED: Write the header-row grammar tests at the two mandated interiors, 78 and 58 — artifact-folds :: `A narrow pane truncates the label and keeps the glyph`, plus the 0..=20 width sweep the scenario names.
- [x] 5.2 RED: Write artifact-content :: `A foldable tab's body is headers, and an open section's markdown beneath its own`, `A non-foldable tab is byte-identical to today`, and `The tracked-tasks tab concatenates rather than folding`.
- [x] 5.3 RED: Write artifact-folds :: `Each drawn header row resolves to its own index` and `Resolution is total and inert where it should be`.
- [x] 5.4 GREEN: Change `content_lines`' return type to `Vec<ContentRow>` — a `markdown::Line` plus a `ContentKind` of `Problem`, `Body`, or `SectionHeader { section, selected }` — and update its two production callers, `ui::view::render`'s detail draw and `Dashboard::normalise_scroll` (which needs only `.len()`), per design.md → Decision 12. `ui::detail` names no `Role` and no `ratatui` type; confirm with `grep -n 'Role::' src/ui/detail.rs` returning nothing.
- [x] 5.5 GREEN: Emit header rows and per-section bodies, gated on `sections.len() > 1`, with the tracked-tasks branch concatenating (design.md → Decisions 3 and 8). The glyph pair is the list region's, per Decision 9 — `pane-chrome` landed, so that is `▸` collapsed and `▾` open. **`section_row_text` is private**, so the pair is extracted into `ui::list::fold_glyph(collapsed: bool) -> char`, `pub(crate)`, called by both it and this header row; read it from there rather than writing a literal, and assert the two **agree** by comparing the two function results. This is the group's one edit to `src/ui/list.rs` and the only reason it is in scope.
- [x] 5.6 GREEN: Implement `ui::detail::section_at(rows, offset, row)` as a **lookup** into the caller's own row list (design.md → Decision 12). It takes **no** `Rect` — `scripts/gates/notabseam.sh` greps this whole file for one — and `offset + row` saturates.
- [x] 5.7 GREEN: Extend the two existing width properties — artifact-content :: `content_lines` is total and width-parameterised (seven `Detail` values to nine) and `No content_lines line exceeds its width at any width` (the 0..=130 sweep over the foldable values).
- [x] 5.8 CHECK: `/bin/sh scripts/gates/notabseam.sh`, `/bin/sh scripts/gates/colwidth.sh`, and `/bin/sh scripts/gates/noio-view.sh` OK — no `.chars()` measurement and no I/O added to a pure view file. Plant `label.chars().count()` in `src/ui/detail.rs`, confirm `COLWIDTH` exits non-zero, remove it, confirm it goes quiet.
- [x] 5.9 CHECK: `/bin/sh scripts/gates/detailwidths.sh` OK — every new test in `ui::detail` names both 58 and 78, which is what the gate counts.
- [x] 5.10 Run the group tests — no regressions, and state that no refactor was needed or name the one performed. **`a_fold_reads_no_file` does not go green here**, and that is the plan's own expectation rather than a miss: group 9 is the Outer Loop GREEN step and follows group 8. Its read-log assertion passes — header rows render correctly — but its four-`Space` tail needs group 7's route-dependent `ToggleSection`, because today the first `Space` at `Route::Detail` folds the *list*'s active section, moves `selected` onto that header, and `sync_detail` then clears `detail.sections` through its nothing-selected branch.

## 6. The drawn slice and the header styling
<!-- kind: behavior -->

- [x] 6.1 RED: Write view-palette :: `A section header's role is selected by its kind, not by its face`. The selected row is asserted against `palette::style`; the unselected rows against their `kind`, since plain `BOLD` cannot discriminate them from `Role::Strong`.
- [x] 6.2 RED: Write artifact-folds :: `The specs tab opens as a list of capability names`, `Folding one section shows its body and leaves its siblings shut`, `The cursor's section header is the emphasised one`, and `An index past the end folds shut rather than panicking`, all at 120x20 and 60x20.
- [x] 6.3 RED: Write the four clauses the extended scenarios gain. **They do not all live in `src/ui/view.rs`**: per design.md → Test Strategy, `A wide-character document stays inside the detail region` is `ui::detail::tests`' and its CJK-labelled foldable leg is a `content_lines` width assertion in `src/ui/detail.rs`, while the other three are `ui::view::tests`'. A per-group file manifest that assumes one file per group is wrong here — the four clauses span three files — artifact-content :: `A read failure is named above the content at both widths`'s no-header-row clause and `A wide-character document stays inside the detail region`'s CJK-labelled foldable case, and view-palette :: `A monochrome reading of the frame is unchanged`'s exactly-one-`REVERSED`-row leg and `Faces reach the buffer as coloured styles at both mandated widths`'s no-`REVERSED` clause.
- [x] 6.4 GREEN: In `ui::view::render`, choose `layout::viewport` for a foldable artifact and `layout::scroll_offset` otherwise (design.md → Decision 2), and map each row's `ContentKind` to `DetailSectionSelected`, `DetailSection`, or no role, patching it over `style_for(&segment.face)` (Decisions 10 and 12). This is the only place the mapping lives, beside the `RowKind` → `Role` match already in this file.
- [x] 6.5 GREEN: Re-assert on `sections` every existing view scenario the matrix marks so — sixteen rows across `artifact-content`, `detail-scroll`, and `responsive-layout`, listed in design.md → Test Strategy. Mechanical field renames, not new assertions, which is why they sit here and 6.1–6.3's new clauses do not.
- [x] 6.6 CHECK: `/bin/sh scripts/gates/palette.sh` OK — every style assertion compares against `palette::style(role)` and no colour literal appears outside `src/ui/palette.rs`'s own tests.
- [x] 6.7 Run the group tests — no regressions, and state that no refactor was needed or name the one performed. **One was performed, across groups rather than inside this one:** foldability was being asked inline at two production sites by the end of group 6 (`src/ui/detail.rs`, `src/ui/view.rs`), and groups 7 and 8 would each have added another. `Detail::foldable()` now decides it once — done here rather than at task 10.1, which is where the review would otherwise have found it after four sites existed. Behaviour-preserving: 1244/2 before and after.
...
- [x] 8.1 RED: Write mouse-input :: `A click on an artifact-section header folds it exactly as Space does`, `A click in an open section's body moves the detail cursor and folds nothing`, and `A click on a non-foldable tab's content is inert` — each at **120x40 and 60x40**, per `openspec/config.yaml` → `rules.tasks`, since the narrow one-region layout is exactly the geometry `Zone`'s split changes.
- [x] 8.2 RED: Extend mouse-input :: `Clicks that address nothing are inert` with a press below the last drawn line, and `The other buttons and the non-press kinds are inert` with a press on a header row.
- [x] 8.3 RED: Extend `ui::layout::zone`'s four existing tests for the sixth variant — responsive-layout :: `The zones tile the frame at 120 columns` (the first and last content rows, and `DetailRow.content` derived independently), `Below the breakpoint only the routed region has zones` (the narrow `DetailRow` leg), `Degenerate frames resolve without panicking` (no zero-sized `DetailRow`), and `The hit test agrees with what was drawn` (`DetailRow` cells checked against `content_lines`' output at the drawn offset).
- [x] 8.4 GREEN: Add `Zone::DetailRow { content, row }` and narrow `Zone::Detail` by the content area in `src/ui/layout.rs`, mirroring `Zone::ListRow { interior, row }`.
- [x] 8.5 GREEN: Add `Target::DetailLine(usize)` and `Target::DetailHeader { line, section }` and resolve them in `mouse_action`.
- [x] 8.6 GREEN: Restructure `apply_click` so the `targets()` membership guard applies only in the `Target::Section` and `Target::Change` arms — the two **list** targets `targets()` yields (design.md → Decision 7). This line previously said "`ArtifactSection` and `Change`"; `ArtifactSection` is the per-file section struct on `Detail` and is not a `Target` variant at all, so the name was a slip for `Target::Section`. Left at the top of the function it returns before either new arm is reached and every detail click is silently inert — 8.1's tests are what catch that. Route the header arm through the very code the detail-route `ToggleSection` runs.
- [x] 8.7 CHECK: `mouse_action`'s existing totality test (`Resolution is total over adversarial geometry`) still passes with the new zone — it sweeps every kind at seven columns, five areas, and both routes, so a new variant that panics is caught there.
- [x] 8.8 CHECK: Contract gate — update `SPEC.md`'s mouse-binding table and confirm `mouse_bindings_match_spec_md` agrees. Its subject is the backticked `Action::` variant set, not the gesture rows, so it cannot fail for this change's new rows (both resolve to `Action::Click` / `Action::Ignore`, already documented): delete one documented row, confirm it fires, restore it, and record both halves — otherwise the check is believed and guards nothing.
- [x] 8.9 Run the group tests — no regressions, and state that no refactor was needed or name the one performed.

## 9. Acceptance Test — Outer Loop GREEN
<!-- kind: behavior -->

- [x] 9.1 VERIFY: Confirm `a_fold_reads_no_file` and `a_foldable_tab_is_clamped_to_its_last_line` pass end to end at both sizes.
- [x] 9.2 REFACTOR: **It does not duplicate one.** `src/ui/driver.rs`'s existing loop helpers each build a single-artifact dashboard (`dashboard_with_one_artifact_path`, `twenty_line_detail_dashboard`, `twenty_task_detail_dashboard`); none builds a **multi-path** artifact, which is the whole point of this fixture, and none returns a read log beside the buffer. `run_three_section_loop` and its `AcceptanceRun` are already the shared shape the three acceptance scenarios reach through, so there is nothing left to fold. Then re-run `cargo test --lib ui::driver::tests` and confirm both acceptance tests are still green.
...
- [x] 10.3 VERIFY: Confirm no blocking or unowned finding remains. **Outcome: 2 CRITICAL, 3 WARNING, 5 SUGGESTION.** Both CRITICALs fixed (`613e6b9`, `91da092`); all three WARNINGs fixed (`803e4b7`); of the SUGGESTIONs, three acted on (glyph requirement amended rather than adding a tautological test, both `refresh.requested` clauses exempted from the blanket archived-tier rule, three stale artifact facts corrected), one **withdrawn by the reviewer** on re-reading (the double row-list derivation is required by `artifact-folds`' recompute-after-the-fold clause; a comment now says why it is a second call), and one **overtaken** by the width repair (the `u16::MAX` allocation). Nothing blocking or unowned remains.

## 11. Documentation
<!-- kind: operational -->

- [x] 11.1 Rewrite in `SPEC.md`: the `Detail` field list and the key-binding table's `Space` row (audience: this repository's future changes) — `Detail` carries sections and a fold set, and `Space` is route-dependent. The mouse-binding table is task 8.7's, which must edit it in the same group or `mouse_bindings_match_spec_md` goes red at 8.8. Replaces the current text; nothing is appended beside it.
- [x] 11.2 Rewrite in `CLAUDE.md`: the paragraph describing the detail region's content, and the one listing the pure view set's width rule (audience: every agent session). It currently says the tab's content is "read through an injected reader … and resolved once per `(change directory, tab)`" with no mention of sections; correct it to name the per-file sections and the one derived foldability predicate, and delete the now-false claim that `j`/`k` "scroll the detail content" unconditionally. Net change must be near zero — this corrects two existing sentences rather than adding a third.
- [x] 11.3 Add one row to `openspec/IMPLEMENTATION-ORDER.md`'s Phase 6 table, marking this change unplanned post-roadmap work as `doc-conformance` is (audience: whoever reads the roadmap next), so the roadmap does not silently stop recording what shipped.

- [x] 11.4 Rename the deleted field in the twelve spec sites this change does not otherwise touch (audience: every future reader of those specs) — `openspec/specs/tasks-checklist/spec.md` 42/58/69/230/238/272/283, `tasks-progress-bar` 84/201, and `live-updates` 244 describe `detail.source`, which will not exist. Their behaviour is unchanged, so this is a prose rename rather than a delta: carrying five behaviourally-identical requirements as MODIFIED blocks would be several hundred lines of copy for a field name. Verify with `grep -rn 'detail\.source' openspec/specs/` returning nothing.
- [ ] 11.5 **Half done; the rest is due immediately after `openspec archive`, following `archive/2026-09-09-list-sections/tasks.md` 9.6's shape exactly — that task also reads "After `openspec archive` applies these deltas".** The three now-false Purposes are rewritten and `cargo test --test spec_purposes` and `openspec validate --specs --strict` both pass (44 items): `detail-scroll` now says what `detail.scroll` *means* on each side of foldability and names `drawn_width` as the one stored geometry; `artifact-content` says one section per resolved file with no separator; `mouse-input`'s click list includes the detail content area and what stays inert there. **Still owed:** `artifact-folds`' own `## Purpose`, which cannot be written yet — the file does not exist until `openspec archive` creates it, and archive writes the `TBD - created by archiving change` placeholder that `tests/spec_purposes.rs` rejects. Write it in the same session as the archive, or that test goes red. Original text: Write `## Purpose` for the new `artifact-folds` capability and rewrite three now-false ones (audience: `openspec validate --specs --strict` and every future reader). `openspec archive` writes the `TBD - created by archiving change` placeholder that `tests/spec_purposes.rs` rejects, and a delta carries requirements rather than a Purpose, so archiving cannot fix these on its own. The three to correct: `openspec/specs/detail-scroll/spec.md` (it names `layout::scroll_offset` as the drawn-window derivation and `j`/`k` as moving it by a line at the detail route — both now conditional on foldability), `openspec/specs/artifact-content/spec.md` (it says the reader concatenates multiple paths in resolution order), and `openspec/specs/mouse-input/spec.md` (its click list omits the detail content area). Follow `archive/2026-09-09-list-sections/tasks.md` 9.6's shape.

## 12. Lint & Verify
<!-- kind: operational -->

- [x] 12.1 CHECK: Confirm the affected tiers are the four design.md → Test Strategy names, and that no test added in this change enters raw mode, spawns a process, or calls `ui::read_artifact`.
- [x] 12.2 VERIFY: `cargo fmt --all -- --check` — clean.
- [x] 12.3 VERIFY: `cargo clippy --all-targets --all-features -- -D warnings` — 0 errors.
- [x] 12.4 VERIFY: `make gates` — every gate OK. `OPENSPEC-UNTOUCHED` requires the change's artifacts committed first; if it still fails, the failure must name a file this change did not create.
- [x] 12.5 VERIFY: `cargo test --all-features` on an idle machine — green **single-threaded** (1258 lib + 19 cli + 61 doc_contract + 5 gate_controls + 4 manifest + 4 degraded_coverage + 3 spec_purposes, exit 0). Under **parallel** `cargo test` in this working tree, nine `ui::tests::wiring` tests fail — **not this change, and not the documented load flake either.** Measured five ways: baseline code `08025d3` also fails in this tree, while byte-identical HEAD code passes 29/29 in a clean `git worktree` (3.72s vs 31s). A **single** wiring test alone costs 5.27s here against 0.53s in the worktree, so it is a ~10x per-test cost in this checkout, not contention — and 5.27s sits on `crate::testutil::Stages`' five-second deadline, which is why any parallelism tips it over. The mechanism is **not** identified: the production watch is narrowed to `<repo>/openspec` and never the repository root, so the 5.6 GB `target/` is not obviously in any scanned path. Recorded as an open environmental question about this checkout rather than a resolved one. Original: `cargo test --all-features` on an idle machine — green. A failure inside `ui::tests::wiring` carrying the empty-log deadline signature is the known load flake; re-run it alone to confirm, and record which it was. A failure anywhere else, or one inside that module with a non-empty call list, is this change's.
- [x] 12.6 VERIFY: `cargo llvm-cov --fail-under-lines 80`, and the production-slice floor from the same run. **Total 95.88% lines** (63720 regions, 2639 missed) against the 80% floor, and **`COVERAGE-PROD OK: production 96.05%` (4593/4782) against its 96% floor** — the falsifiable one, and the slice this change adds to. Per-file for the files this change touched: `ui/view.rs` 94.17%, `ui/mod.rs` 94.88%, `ui/list.rs` 95.28%, `ui/layout.rs` 94.74%, `ui/palette.rs` 91.11%. No floor was lowered, waived, or given an exclusion.
- [x] 12.7 VERIFY: `make check` as the single gate — **exit 0**, run in a clean `git worktree` at this change's HEAD. In the **main** checkout it exits 2 at the `test` step for the environmental reason 12.5 records, and because `test` precedes `coverage` in `check:`'s prerequisite list, that failure also prevents the coverage floor from ever being measured there. The worktree run is the only place the composite gate gives a true reading on this machine; CI runs on a fresh checkout and does not meet the condition.
- [x] 12.8 VERIFY: `openspec validate foldable-spec-sections --strict`.
- [x] 12.9 VERIFY: Every `cargo test --lib` filter in design.md → Test Strategy selects at least one test. Extract them and check each against `cargo test --lib -- --list`; **zero matches is a failure**, because a filter that matches nothing exits `0` and reads as a passing verification. This is the check whose absence let about thirty-seven dead filters survive a planning review that recorded the defect class as repaired (design.md → Test Strategy, "How to read the Command column"). Report the count checked and the count matched.
