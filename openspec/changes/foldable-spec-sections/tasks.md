<!-- Planning-time measurements. Every number below names the command that produced it,
     run against HEAD (027db45) on 2026-09-09. -->

## Baseline recorded at HEAD
...
| `Detail { … }` literal/pattern spans | `grep -rn 'Detail {' src/ \| grep -v 'struct Detail' \| wc -l` | **60** — every one must name the new field |
| `.source` references under `src/ui/` | `grep -rc '\.source\b' src/ui/*.rs \| grep -v ':0'` | app.rs 31, detail.rs 7, driver.rs 6, view.rs 5, mod.rs 3, tasks.rs 1 |
...
| R5 — `Space` is route-blind today | `grep -n 'Action::ToggleSection => self.apply_toggle_section(),' src/ui/app.rs` | **exit 0**, line 433 — the unconditional arm group 6 replaces |
| Hygiene gates | `make gates` | **exit 2**, failing only `OPENSPEC-UNTOUCHED`, and naming only this change's own untracked artifacts. Every other gate printed OK. Committing them clears it. |
| Test suite | `cargo test --all-features` | **Green at HEAD on an idle machine: 1222 passed, 0 failed.** Two runs taken under CPU contention gave 1 and 8 failures, every one inside `ui::tests::wiring`; each passed in isolation. See below. |
...
- [ ] 1.1 RED: Extend `src/ui/palette.rs`'s three existing table tests for the two new roles — view-palette :: `The palette answers every role with a Style` (including the `DetailSectionSelected` inequality assertion), `Each role's modifier set is exactly the table above` (`BOLD | REVERSED`, not `BOLD` alone; the no-modifier count stays nine), and `The coloured set is exactly the table above` (both report `fg: None`, `bg: None`).
...
- [ ] 5.5 GREEN: Emit header rows and per-section bodies, gated on `sections.len() > 1`, with the tracked-tasks branch concatenating (design.md → Decisions 3 and 8). The glyph pair is **whatever `src/ui/list.rs`'s `section_row_text` uses at that moment**, per Decision 9 — `>`/`v` today, `▾`/`▸` if `pane-chrome` has landed first. Read it from that function rather than writing a literal.
...
- [ ] 10.1 CHECK: Dispatch `outside-in-tdd-reviewer` — a fresh agent, not a fork of the implementing session — against proposal.md, all five spec files, design.md, tasks.md, and the diff. Point it first at: whether any of the 111 scenarios has a test that could not go red; whether `sections.len() > 1` is asked in one place or several; and whether the two `Detail` fields are named at all 60 spans.
