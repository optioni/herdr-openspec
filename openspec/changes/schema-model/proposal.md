...
- **No process spawn, no `cli` module, no CLI fallback.** `openspec schema which` needs the
  ...
  there is no ordered chain here whose position needs pinning — `design.md` → Decisions
  argues this rather than leaving the asymmetry unexplained.
...
- `plugin-build`: the declared-dependency-set requirement names exactly one crate and pins
  ...
  the MSRV clause gains the scenario it never had, because `yaml-rust2`'s `1.85.0` becomes
  the tightest MSRV in the graph and equals this crate's own.

  The delta is written as **REMOVED plus ADDED** rather than MODIFIED, and this is the
  one place the tool constrains the artifact: the count is written into a scenario
  *heading* ("The declared dependency set is exactly one crate"), and
  `openspec validate --strict` rejects a MODIFIED requirement that omits any scenario name
  the live spec still carries — a MODIFIED block cannot rename a scenario. Keeping the
  heading would archive a live spec whose scenario is named "exactly one crate" while its
  own THEN asserts two. The replacement is named without a count, carries every clause and
  every scenario of the removed requirement, and the REMOVED block states this as its
  Reason. `plugin-build` keeps its two other requirements, so no capability is retired.

## Impact

- **Code:** new `src/schema.rs`; `src/lib.rs` gains `pub mod schema;` and one `testutil`
  helper for writing a fixture repository tree. `src/main.rs`, `src/config.rs`,
  `src/state.rs`, and `src/resolve.rs` are untouched — nothing consumes the schema until
  `changes-from-files`.
- **Build:** `Cargo.toml` gains `yaml-rust2` with `default-features = false` and an empty
  explicit feature list (defaults off drops `encoding_rs`), and `Cargo.lock` moves. The
  normal graph gains exactly `yaml-rust2`, `arraydeque`, `hashlink`, `hashbrown`, and
  `foldhash` — no proc macro, no C.
- **Docs:** `SPEC.md` → Overview (the stack line), Data layer → Resolution chain
  ("Schema"), Degraded states, and Testing → Unit-tested modules;
