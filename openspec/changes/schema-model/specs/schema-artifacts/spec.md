...
segment, not the file's own `name:` key. Those two are normally equal; when they differ the
difference SHALL be recorded as a problem, because a forked schema whose directory was
renamed still resolves by directory and the mismatch is otherwise invisible.
...
- **unreadable** — the path exists but the bytes could not be obtained;
- **invalid** — the bytes were read but are not a usable schema.
...
the three reasons, and the problems.
...
- **THEN** the schema loads, its name is `tdd`, and its artifact list is non-empty
- **AND** the first artifact's id is `proposal`
- **AND** the artifact list contains an artifact with id `tasks` and `generates`
  `tasks.md`, and one with id `specs` whose `generates` is the glob `specs/**/*.md`, carried
  verbatim rather than expanded
- **AND** no problem is recorded

#### Scenario: Artifact order is the file's order, not alphabetical

- **WHEN** a fixture schema lists artifacts in the order `zeta`, `alpha`, `middle`
- **THEN** the produced list is exactly `zeta`, `alpha`, `middle`
- **AND** it is neither sorted nor reversed, which a single-artifact fixture could not
  distinguish
...
This is the rule the OpenSpec CLI itself applies in `findTrackedTasksArtifact`.
**`SPEC.md`'s claim that the artifact carries a `role: tasks` key is false** — no `role` key
exists in the schema format, in the vendored `tdd` schema, in the CLI's bundled
`spec-driven` schema, or in the CLI's own definition of an artifact. `SPEC.md` is corrected
by this change.
...
containing no `..` segment and no NUL byte — the same constraint the OpenSpec CLI applies to
`generates`, and one worth applying here because `changes-from-files` joins that value onto
a change directory.
...
renders none of them and failing on a field it never uses would fail closed.
...
A schema file SHALL be treated as *invalid* — no schema produced, one problem recorded
naming the path and carrying the underlying reason — when the bytes are not valid YAML, when
the file holds no document at all, when the first document is not a mapping, when
`artifacts:` is absent, when `artifacts:` is not a sequence, or when `artifacts:` is an empty
sequence.
...
- **THEN** the reason is **invalid**, naming the path, and the recorded problem carries the
  parser's message so the offending line can be found
- **AND** the call returns rather than panicking
...
- **THEN** each yields the **invalid** reason with exactly one problem
