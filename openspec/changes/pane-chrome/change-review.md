...
- `src/ui/view.rs` — `render_region` recomputes the list interior by hand as
  ...
  that produced findings 1 and 2. **Folded into the `detail_gutters` repair.**
...
with no duplicates. Both used the same corpus. The discrepancy is unresolved and is left to the
conformance check at `tasks.md` → 10.2, which is the arbiter: it runs every command in the
matrix and fails on any that runs zero tests.
