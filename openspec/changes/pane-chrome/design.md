...
| artifact-content → The rendered markdown fills the content area, not the whole interior | `the_rendered_markdown_fills_the_content_area_not_the_whole_interior` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_rendered_markdown_fills_the_content_area_not_the_whole_interior` |
| artifact-content → The tracked-tasks tab renders the checklist body instead | `the_tracked_tasks_tab_renders_the_checklist_body_instead` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_tracked_tasks_tab_renders_the_checklist_body_instead` |
...
| artifact-content → `content_lines` is total and width-parameterised | `content_lines_is_total_and_width_parameterised` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib content_lines_is_total_and_width_parameterised` |
...
| change-rows → Active rows render at both mandated widths | `active_rows_render_at_both_mandated_widths` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib active_rows_render_at_both_mandated_widths` |
| change-rows → A badged row carries its status between the name and the progress cell | `a_badged_row_carries_its_status_between_the_name_and_the_progress_cell` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_badged_row_carries_its_status_between_the_name_and_the_progress_cell` |
...
| change-rows → A watch problem leads the list, above a change-set problem | `a_watch_problem_leads_the_list_above_a_change_set_problem` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_watch_problem_leads_the_list_above_a_change_set_problem` |
| change-rows → The row grammar places the marker, the name, and the progress cell | `the_row_grammar_places_the_marker_the_name_and_the_progress_cell` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_row_grammar_places_the_marker_the_name_and_the_progress_cell` |
| change-rows → A name too long for the field is truncated with an ellipsis | `a_name_too_long_for_the_field_is_truncated_with_an_ellipsis` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_name_too_long_for_the_field_is_truncated_with_an_ellipsis` |
| change-rows → A field too narrow for both drops the progress cell whole | `a_field_too_narrow_for_both_drops_the_progress_cell_whole` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_field_too_narrow_for_both_drops_the_progress_cell_whole` |
| change-rows → No repository names the directory searched, at both widths | `no_repository_names_the_directory_searched_at_both_widths` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib no_repository_names_the_directory_searched_at_both_widths` |
| change-rows → A repository with no changes at all | `a_repository_with_no_changes_at_all` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_repository_with_no_changes_at_all` |
...
| change-rows → Repository-level problems are named above the rows | `repository_level_problems_are_named_above_the_rows` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib repository_level_problems_are_named_above_the_rows` |
...
| change-rows → More changes than rows do not overflow the region | `more_changes_than_rows_do_not_overflow_the_region` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib more_changes_than_rows_do_not_overflow_the_region` |
...
| detail-header → Moving the selection moves the header | `moving_the_selection_moves_the_header` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib moving_the_selection_moves_the_header` |
...
| detail-header → A CJK change name keeps the header inside its region at both mandated widths | `a_cjk_change_name_keeps_the_header_inside_its_region_at_both_mandated` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_cjk_change_name_keeps_the_header_inside_its_region_at_both_mandated` |
...
| detail-header → The header is total over adversarial names at every width | `the_header_is_total_over_adversarial_names_at_every_width` | unit (`src/ui/*` inline `#[cfg(test)]`) | none — no I/O, no terminal, no process | `cargo test --all-features --lib the_header_is_total_over_adversarial_names_at_every_width` |
...
| detail-scroll → The document fills the detail interior at both mandated widths | `the_document_fills_the_detail_interior_at_both_mandated_widths` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_document_fills_the_detail_interior_at_both_mandated_widths` |
| detail-scroll → Faces reach the buffer as styles at both widths | `faces_reach_the_buffer_as_styles_at_both_widths` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib faces_reach_the_buffer_as_styles_at_both_widths` |
| detail-scroll → A table reaches the buffer aligned and inside the region | `a_table_reaches_the_buffer_aligned_and_inside_the_region` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_table_reaches_the_buffer_aligned_and_inside_the_region` |
| detail-scroll → An empty source leaves the detail interior blank at both widths | `an_empty_source_leaves_the_detail_interior_blank_at_both_widths` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib an_empty_source_leaves_the_detail_interior_blank_at_both_widths` |
...
| detail-scroll → A scroll offset past the end still draws the last screenful | `a_scroll_offset_past_the_end_still_draws_the_last_screenful` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib a_scroll_offset_past_the_end_still_draws_the_last_screenful` |
| detail-scroll → At the detail route the content scrolls by one line at both widths | `at_the_detail_route_the_content_scrolls_by_one_line_at_both_widths` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib at_the_detail_route_the_content_scrolls_by_one_line_at_both_widths` |
| detail-scroll → At the list route the same actions still move the selection | `at_the_list_route_the_same_actions_still_move_the_selection` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib at_the_list_route_the_same_actions_still_move_the_selection` |
| detail-scroll → Scrolling stops at the top | `scrolling_stops_at_the_top` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib scrolling_stops_at_the_top` |
| detail-scroll → While filtering, `j` and `k` still type into the query | `while_filtering_j_and_k_still_type_into_the_query` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib while_filtering_j_and_k_still_type_into_the_query` |
...
| detail-scroll → `Enter` at the detail route moves nothing and keeps the scroll | `enter_at_the_detail_route_moves_nothing_and_keeps_the_scroll` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib enter_at_the_detail_route_moves_nothing_and_keeps_the_scroll` |
| detail-scroll → `Enter` from the list route still opens at the top | `enter_from_the_list_route_still_opens_at_the_top` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib enter_from_the_list_route_still_opens_at_the_top` |
| detail-scroll → `Enter` while filtering still dismisses the filter and resets nothing | `enter_while_filtering_still_dismisses_the_filter_and_resets_nothing` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib enter_while_filtering_still_dismisses_the_filter_and_resets_nothing` |
| list-filtering → A query narrows both tiers at both widths | `a_query_narrows_both_tiers_at_both_widths` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_query_narrows_both_tiers_at_both_widths` |
...
| list-filtering → Matching ignores case outside ASCII | `matching_ignores_case_outside_ascii` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib matching_ignores_case_outside_ascii` |
...
| list-filtering → Shrinking the visible list clamps the selection | `shrinking_the_visible_list_clamps_the_selection` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib shrinking_the_visible_list_clamps_the_selection` |
...
| list-selection → A selection past the interior scrolls the slice at both widths | `a_selection_past_the_interior_scrolls_the_slice_at_both_widths` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib a_selection_past_the_interior_scrolls_the_slice_at_both_widths` |
| list-selection → The last change is reachable and the slice stops at the end | `the_last_change_is_reachable_and_the_slice_stops_at_the_end` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_last_change_is_reachable_and_the_slice_stops_at_the_end` |
| list-selection → The viewport is exact at its boundaries | `the_viewport_is_exact_at_its_boundaries` | unit (`src/ui/*` inline `#[cfg(test)]`) | none — no I/O, no terminal, no process | `cargo test --all-features --lib the_viewport_is_exact_at_its_boundaries` |
| list-selection → A resize changes the slice on the next frame | `a_resize_changes_the_slice_on_the_next_frame` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_resize_changes_the_slice_on_the_next_frame` |
...
| mouse-input → The two regions scroll independently at 120 columns | `the_two_regions_scroll_independently_at_120_columns` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib the_two_regions_scroll_independently_at_120_columns` |
...
| mouse-input → At 60 columns only the routed region answers the wheel | `at_60_columns_only_the_routed_region_answers_the_wheel` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib at_60_columns_only_the_routed_region_answers_the_wheel` |
...
| mouse-input → A click selects a change row and a second click opens it | `a_click_selects_a_change_row_and_a_second_click_opens_it` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib a_click_selects_a_change_row_and_a_second_click_opens_it` |
| mouse-input → A click on a section header folds it exactly as `Space` does | `a_click_on_a_section_header_folds_it_exactly_as_space_does` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib a_click_on_a_section_header_folds_it_exactly_as_space_does` |
| mouse-input → A click on an archived header opens an unresolved archive and requests its refresh | `a_click_on_an_archived_header_opens_an_unresolved_archive_and_requests` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib a_click_on_an_archived_header_opens_an_unresolved_archive_and_requests` |
| mouse-input → A click on a tab cell switches to that artifact | `a_click_on_a_tab_cell_switches_to_that_artifact` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib a_click_on_a_tab_cell_switches_to_that_artifact` |
...
| mouse-input → The other buttons and the non-press kinds are inert | `the_other_buttons_and_the_non_press_kinds_are_inert` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib the_other_buttons_and_the_non_press_kinds_are_inert` |
...
| responsive-layout → The action hints follow `Esc back` when the socket is reachable | `the_action_hints_follow_esc_back_when_the_socket_is_reachable` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib the_action_hints_follow_esc_back_when_the_socket_is_reachable` |
| responsive-layout → The action hints are dropped whole, `g focus` first | `the_action_hints_are_dropped_whole_g_focus_first` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib the_action_hints_are_dropped_whole_g_focus_first` |
...
| responsive-layout → The footer drops whole hints rather than truncating one | `the_footer_drops_whole_hints_rather_than_truncating_one` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_footer_drops_whole_hints_rather_than_truncating_one` |
| responsive-layout → The unattributed count is the footer's last hint at both widths | `the_unattributed_count_is_the_footer_s_last_hint_at_both_widths` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_unattributed_count_is_the_footer_s_last_hint_at_both_widths` |
| responsive-layout → The count is reported with an empty change list | `the_count_is_reported_with_an_empty_change_list` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_count_is_reported_with_an_empty_change_list` |
| responsive-layout → The count is dropped whole before the three key hints | `the_count_is_dropped_whole_before_the_three_key_hints` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_count_is_dropped_whole_before_the_three_key_hints` |
| responsive-layout → The filter prompt replaces the count along with the hints | `the_filter_prompt_replaces_the_count_along_with_the_hints` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_filter_prompt_replaces_the_count_along_with_the_hints` |
...
| responsive-layout → The divider column is a width branch, not a constant | `the_divider_column_is_a_width_branch_not_a_constant` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_divider_column_is_a_width_branch_not_a_constant` |
...
| responsive-layout → Interiors are blank at both widths | `interiors_are_blank_at_both_widths` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib interiors_are_blank_at_both_widths` |
| responsive-layout → Rows do not overwrite the borders at either width | `rows_do_not_overwrite_the_borders_at_either_width` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib rows_do_not_overwrite_the_borders_at_either_width` |
...
| tasks-checklist → The tasks tab shows checkboxes and its siblings show markdown | `the_tasks_tab_shows_checkboxes_and_its_siblings_show_markdown` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_tasks_tab_shows_checkboxes_and_its_siblings_show_markdown` |
| tasks-checklist → The tab is chosen by `tracks_tasks`, not by its id | `the_tab_is_chosen_by_tracks_tasks_not_by_its_id` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_tab_is_chosen_by_tracks_tasks_not_by_its_id` |
| tasks-checklist → A schema naming no tasks artifact leaves every tab as markdown | `a_schema_naming_no_tasks_artifact_leaves_every_tab_as_markdown` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_schema_naming_no_tasks_artifact_leaves_every_tab_as_markdown` |
| tasks-checklist → A `detail.tab` past the end of the artifact list renders no checklist | `a_detail_tab_past_the_end_of_the_artifact_list_renders_no_checklist` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_detail_tab_past_the_end_of_the_artifact_list_renders_no_checklist` |
| tasks-checklist → A prose-only tasks file reads `No tasks yet` | `a_prose_only_tasks_file_reads_no_tasks_yet` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_prose_only_tasks_file_reads_no_tasks_yet` |
...
| tasks-checklist → A missing tasks artifact still reads `No content yet` | `a_missing_tasks_artifact_still_reads_no_content_yet` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_missing_tasks_artifact_still_reads_no_content_yet` |
| tasks-checklist → A read failure on the tasks tab names its reason and renders no checklist | `a_read_failure_on_the_tasks_tab_names_its_reason_and_renders_no` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib a_read_failure_on_the_tasks_tab_names_its_reason_and_renders_no` |
...
| view-palette → Faces reach the buffer as coloured styles at both mandated widths | `faces_reach_the_buffer_as_coloured_styles_at_both_mandated_widths` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib faces_reach_the_buffer_as_coloured_styles_at_both_mandated_widths` |
