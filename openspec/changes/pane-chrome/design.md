...
| detail-scroll → A scroll offset past the end still draws the last screenful | `a_scroll_past_the_end_draws_the_last_screenful` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib a_scroll_past_the_end_draws_the_last_screenful` |
...
| list-selection → A selection past the interior scrolls the slice at both widths | `a_selection_past_the_interior_scrolls_the_slice` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib a_selection_past_the_interior_scrolls_the_slice` |
...
| responsive-layout → The action hints follow `Esc back` when the socket is reachable | `the_action_hints_follow_esc_back_when_reachable` | unit (`src/ui/app.rs`, `src/ui/driver.rs` inline) | `TestBackend` real; filesystem, Herdr socket and terminal replaced | `cargo test --all-features --lib the_action_hints_follow_esc_back_when_reachable` |
...
| responsive-layout → The divider column is a width branch, not a constant | `the_breakpoint_is_exact_at_99_100_and_101_columns` | unit (`src/ui/*` inline `#[cfg(test)]`) | `TestBackend` real; filesystem, process and terminal absent | `cargo test --all-features --lib the_breakpoint_is_exact_at_99_100_and_101_columns` |
