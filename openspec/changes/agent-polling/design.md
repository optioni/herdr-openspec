...
| dashboard-loop: No test sleeps and then asserts something has already happened | `NOSLEEP` at `SLEEP_MIN=5` (raised from the planned `4` — Change Review's fix for the acceptance test's poller-schedule tests added a fifth, deadline-bounded sleep site), both legs, with its self-contained negative control | check | source tree | `SLEEP_MIN=5 MIN=25 sh $CHECKS/NOSLEEP.sh` |
