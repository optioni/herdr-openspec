...
`id = "herdr-openspec"`, `name = "OpenSpec"`, `min_herdr_version = "0.7.0"`, and
`platforms = ["macos", "linux"]`. It SHALL declare exactly one `[[build]]` step with
...
- **THEN** parsing succeeds and the six required keys above are present with exactly
  those values
