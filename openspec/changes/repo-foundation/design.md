...
| The manifest parses and its declared paths resolve | Parse the manifest with `tomllib`, assert the six required keys carry exactly the specified values, and assert both declared paths are executable | command check | real filesystem, python3 3.11+ | `python3 -c 'import tomllib,os; m=tomllib.load(open("herdr-plugin.toml","rb")); …'` |
