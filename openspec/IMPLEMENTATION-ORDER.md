...
| `spec-purposes` | Fill in the Purpose of every capability under `openspec/specs/`. `openspec archive` writes `TBD - created by archiving change <x>` and nothing replaces it, so `openspec validate --specs --strict` currently fails on 25 of 39 specs. The specs are part of the deliverable; the repository should not ship failing its own validation. | Testing and quality gates | `plugin-actions` |
