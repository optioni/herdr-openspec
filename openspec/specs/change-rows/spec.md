...
1. one `Problem` row per entry of `dashboard.changes.problems`, in that vector's order;
2. the visible **active** changes, in `dashboard.changes.active`'s order, or — when that
...
3. one `Separator` row, **only** when at least one visible archived change follows it;
4. the visible **archived** changes, in `dashboard.changes.archived`'s order.

`rows` SHALL NOT sort, de-duplicate, or re-order either tier: `changes::from_files`
already orders active changes name-ascending in byte order and archived changes
dated-newest-first, and a second ordering rule here would be a second place for the two
producers of `Change` to disagree.
...
- **WHEN** a `Dashboard` whose repository root is `/tmp/demo-repo` and whose
  ...
  at 7 of 7, and `migrate-ai-sdk-v7` at 0 of 0, with no archived changes, no problems, an
  empty filter query, and `selected` 0, is rendered into a `TestBackend` at 120x20 and
  again at 60x20
