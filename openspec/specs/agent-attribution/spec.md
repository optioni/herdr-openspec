...
## Purpose
The mapping from a live Herdr agent to the change it is working on: three tiers —
the plugin-local agent-name mapping, an exact match of an agent's `name` against a
change name, and neither — every tier scoped to the resolved repository, the agent
that satisfies none of them counted rather than assigned, the badge status chosen
when several agents share one change, and the whole derivation kept pure, recomputed
per frame, keyed by change name, and stored nowhere.
