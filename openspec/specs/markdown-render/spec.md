...
thematic breaks, a soft break folded into a single space so the paragraph reflows at the
region width while a hard break still starts a line, and one blank line
...
`~~struck~~` sets a face on its text. A task-list item is modelled, carrying `[✓] `/`[ ] ` in
place of its bullet marker and hanging its continuations under its own four-column text
position. The one construct the parser is deliberately not configured for — a footnote —
renders as its literal source rather than vanishing. Every glyph the renderer emits that the
source does not carry measures exactly one display column, asserted rather than assumed, and
