...
## Purpose
The `,` overlay itself: when it opens, what it renders, which keys it
takes while it is up, how it closes, and how it degrades at the narrow layout. The panel is the
crate's **second** overlay panel rather than a second overlay layer — `Overlay { panel, scroll,
edit }` carries `Option<Panel>`, so `?` and `,` share one dismissal, one scroll primitive and one
mouse-capture rule, and a click outside the band resolves to `Action::Back` whichever panel is
up. It is the one place the pane accepts a write from the user: every other surface is read-only,
and the commit that writes `settings.toml` is specified here rather than at the state seam.
