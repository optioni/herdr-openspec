...
## Purpose
`?` opens a read-only overlay listing every key and gesture the pane binds, drawn over the
body as a full-width band and dismissed by `?`, `Esc`, or a click outside it.

It exists because the footer cannot hold the pane's bindings. The footer is the
always-visible minimum and drops hints whole from the end as the frame narrows: at the
mandated 60 columns it sheds `g focus`, and with an unreachable socket it sheds the launch
keys entirely. Accepting those drops is only defensible because the overlay lists them
anyway — `INVENTORY` is `'static` and no render path under `ui::help` consults
`agents.reachable`, so the overlay shows the same bindings whatever the socket is doing.

The overlay is a **band**, not a box, and its scroll position is digits rather than arrow
glyphs. Six of the seven box-drawing characters and both arrows are East Asian
**Ambiguous**, which a CJK-locale terminal paints at two columns where `layout::columns`
says one; `─` alone is not, so the band's two rule rows are the only chrome it draws.

It is read-only in the strong sense: while it is open, `apply` routes seven actions to the
overlay itself and the other seventeen are inert by name, so no key reaches the dashboard
underneath. A click inside the band does nothing — no row is a button — and a click
outside it dismisses the overlay without also selecting what it landed on.
