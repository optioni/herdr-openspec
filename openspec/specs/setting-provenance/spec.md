# setting-provenance Specification

## Purpose
For each setting, the effective value together with the precedence
level that produced it, as a value the view **renders** rather than derives. The panel draws
provenance; it never recomputes it. That split is what keeps `src/settings.rs` pure and outside
`src/ui/`, and what lets the `agent_kind` row show `Pending` honestly for the frames between `,`
being pressed and the launcher's worker answering `Request::Resolve` — a state a view deriving
its own provenance could not represent without blocking the render path on `herdr integration
status`.
