...
| Left click outside the help overlay's band while the overlay is open (`overlay.panel`) — above it, below it, or on the footer row | `Action::Back` — close whichever panel is open, and nothing else in the same event: the click that dismissed it does not also select the row under it. **Not** `Action::ToggleHelp`: with two panels sharing this layer, `ToggleHelp` means *swap to help* rather than *close* (`settings-window`'s correction to `help-overlay`) |
