...
|---|---|
| `config` | Resolve the plugin's configuration directory from the process environment (no process spawn) and read `config.toml` into `openspec_bin`, `agent_kind`, `archived_count`, and the per-kind `prompts` overrides |
...
| `,` | Open the settings panel when it is closed, close it when it is open — at either route, and without moving it. While filtering, `,` types itself into the query instead, like every other printable key |
...
| Left click on a setting's **value** or **source** row inside the band, while the settings panel is the open one (`overlay.panel` is `Some(Panel::Settings)`) | `Action::Click` naming `Target::Setting` for that setting's own index — address the setting by either of its two rendered rows and move the panel's row cursor to it, exactly as `j`/`k` do inside the band. The band's heading row, either of its rule rows, and a row the panel left blank all resolve to Ignore, as does every click inside the **help** panel's band, which is read-only and holds no control (`settings-window`) |
| Left click outside the help overlay's band while the overlay is open (`overlay.panel`) — above it, below it, or on the footer row | `Action::Back` — close whichever panel is open, and nothing else in the same event: the click that dismissed it does not also select the row under it. **Not** ToggleHelp — written without the `Action::` qualifier so `backticked_action_variants`' no-negation-aware scan does not read this row as also covering it: with two panels sharing this layer, `ToggleHelp` means *swap to help* rather than *close* (`settings-window`'s correction to `help-overlay`) |
| Wheel down anywhere in the frame while the overlay is open (`overlay.panel`) | `Action::ScrollDown` — scroll the overlay itself rather than the region under the pointer, which is covered by the modal (`help-overlay`) |
| Wheel up anywhere in the frame while the overlay is open (`overlay.panel`) | `Action::ScrollUp`, on the same terms |
...
While the help overlay is open the four rows above that carry `overlay.panel` take
...
| `herdr integration status` cannot be read (non-zero exit, a failure to start, or a timeout) | The launch **still runs**. Resolution continues as though no integration were reported, so a configured or recorded kind is honoured — with no warning about its own integration, because an absence cannot be established from a read that failed — and with neither set the kind is `claude` under the last resort. Herdr's own reason is reported as one `!`-marked problem row, ahead of the launch's own, on the launch that read the status and on no other — the read happened once. Output that exits 0 but parses to no integration at all degrades identically, except that the one entry is a **summary** naming how many lines could not be read rather than one entry per line, so a format change costs one row and not seventeen |
| Two or more agent integrations installed, with no `agent_kind` configured and none recorded | The **one** resolution outcome that stops a launch: no `pane split` is issued, no pane is created and none can be orphaned, `named` is `None`, and one `!`-marked row asks for an explicit `agent_kind` and names every installed kind in Herdr's own printed order — in that order, front-loaded, because the row is cut to the list region's 38-column interior and an explanation-first wording put both candidates past the cut. `launch.in_flight` is cleared by the same `drain` that adopts it, so the keys are not locked for the session, and `g` still focuses. Not a contradiction of "never fail closed": the evidence exists and points two ways at once, and choosing between two clients the reader has both set up is exactly the guess `agent-attribution` refuses to make about a terminal title (`settings-window` upgrades this row to a picker) |
| No agent integration installed at all, with no `agent_kind` configured and none recorded | `claude` is launched as the **last resort** and one `!`-marked row says so, on the launch that resolved the kind — the resolution happens once per session and its warnings are reported once, unlike the ambiguous refusal above, which is re-derived from the cached choice on every press because it refuses rather than warns. The launch is not refused — there is no evidence at all here, and refusing would fail closed — but the one genuinely blind guess stops being silent |
| The chosen agent kind's own integration is not installed (a configured or recorded kind Herdr reports as absent, or never lists) | The launch **still runs** under that kind, with one `!`-marked row naming it, on the launch that resolved the kind, and warning that every agent started under it will report its status as `unknown`. This is not an availability check: `integration status` reports whether Herdr's status-reporting hook is installed, not whether the client's executable exists, and `agent start` fails with its own reason on a kind it cannot launch |
